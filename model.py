...
class MyModel(nn.Module):
    ...
    def _loss(self, input, mask, start_target, end_target, spans):
        ...
        start_logits = torch.cat([start_logits[i][mask[i]]] for i in range(batch_size))
        end_logits = torch.cat([end_logits[i][mask[i]]] for i in range(batch_size))
        start_target = start_target.masked_select(start_target)
        end_target = end_target.masked_select(end_target)
